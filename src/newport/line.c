...
static void
benchmark_line(struct gfx_ctx *ctx, int tw, int th, int tcount)
{
	...
	for (int i = 0; i < tcount; i++) {
		...
		newport_draw_line(ctx, x, y, x + w, y + h, c);
	}
	...
	/*
	 * hard to do pixels per sec when I'm not tracking per line
	 * length above, so just 0 for now.
	 */
...
main(int argc, const char *argv[])
{
	...
	if (strcmp(mode, "benchmark") == 0) {
		...
		benchmark_line(&ctx, 8, 8, arg2);
		benchmark_line(&ctx, 16, 16, arg2);
		benchmark_line(&ctx, 32, 32, arg2);
		benchmark_line(&ctx, 64, 64, arg2);
		benchmark_line(&ctx, 128, 128, arg2);
	} else {
